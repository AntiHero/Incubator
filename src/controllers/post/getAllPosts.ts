...
export const getAllPosts = [...validatePaginationQuery, async (req: Request, res: Response) => {
    const {
      pageNumber,
      pageSize,
      sortBy,
  ...
  const posts = await postsRepository.findPostsByQuery({pageNumber, pageSize, sortBy, sortDirection, searchNameTerm});
